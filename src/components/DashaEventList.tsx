...
export default function DashaEventList({ planets, ascendant, birthDatetime, dashaSettings, transitPlanets = [], transitDatetime = '', onSetTransitDatetime, onOpenVargaMatrix, mode = 'events' }: Props) {
  ...
  return <section className="space-y-3 rounded-lg border border-zinc-200 bg-zinc-50/70 p-3 dark:border-zinc-700 dark:bg-zinc-950/40">
    ...
    {mode === 'patterns' && events.length >= 2 && <details open className="rounded-md border border-cyan-200 bg-cyan-50/40 dark:border-cyan-900 dark:bg-cyan-950/10">
      ...
      {patterns[0]?.sampleSize < 5 && <p className="rounded border border-amber-200 bg-amber-50 p-2 text-[10px] text-amber-800 dark:border-amber-900 dark:bg-amber-950/20 dark:text-amber-300">Small sample ({patterns[0]?.sampleSize ?? 0} events): patterns are exploratory.</p>}{patterns.length ? <div className="grid gap-1 sm:grid-cols-2">{patterns.map(pattern => <div key={pattern.key} className="flex items-center gap-2 rounded border border-cyan-100 bg-white px-2 py-1.5 text-[10px] dark:border-cyan-900 dark:bg-zinc-900"><span className="min-w-0 flex-1 truncate"><span className="text-zinc-500">{pattern.dasha} · {pattern.level}</span> <span className="font-semibold text-zinc-800 dark:text-zinc-100">{pattern.value}</span><span className="ml-1 text-[8px] uppercase text-zinc-400">{pattern.kind}</span></span><span className="rounded bg-cyan-100 px-1.5 py-0.5 font-mono text-cyan-800 dark:bg-cyan-950 dark:text-cyan-300">{pattern.count}/{pattern.sampleSize} · {Math.round(pattern.support * 100)}% · {pattern.confidence}</span></div>)}</div> : <p className="text-[10px] italic text-zinc-400">No period repeats in this category yet. At least two matching events are required.</p>}
