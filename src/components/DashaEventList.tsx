...
export default function DashaEventList({ planets, ascendant, birthDatetime, dashaSettings, transitPlanets = [], transitDatetime = '', onSetTransitDatetime, onOpenVargaMatrix, mode = 'events' }: Props) {
  ...
  return <section className="space-y-3 rounded-lg border border-zinc-200 bg-zinc-50/70 p-3 dark:border-zinc-700 dark:bg-zinc-950/40">
    ...
    <div className="flex flex-wrap gap-2"><input value={search} onChange={event => setSearch(event.target.value)} placeholder="Search names, notes or tags" className="min-w-[220px] flex-1 rounded border border-zinc-300 bg-white px-2 py-1.5 text-xs dark:border-zinc-700 dark:bg-zinc-900"/><label className="text-[10px] text-zinc-500">Min. significance <select value={minimumSignificance} onChange={event => setMinimumSignificance(Number(event.target.value))} className="ml-1 rounded border border-zinc-300 bg-white px-2 py-1.5 dark:border-zinc-700 dark:bg-zinc-900">{[1,2,3,4,5].map(value => <option key={value}>{value}</option>)}</select></label></div>
    </>}
