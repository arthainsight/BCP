...
export default function RasiDashaPanel({ system, planets, ascendant, birthDatetime }: { system: RasiDashaSystem; planets: PlanetData[]; ascendant: { sign: number }; birthDatetime: string }) {
  ...
  return <div className="space-y-3 min-w-0">
    ...
    <div className="space-y-1">{rows.map((entry) => { const active = entry.startDate <= now && now < entry.endDate; return <button key={`${entry.sign}-${entry.startDate.getTime()}`} type="button" onClick={() => open(entry)} className={`w-full rounded-md border px-3 py-2 text-left font-mono text-xs ${active ? 'border-cyan-400 bg-cyan-50 text-cyan-800 dark:border-cyan-600 dark:bg-cyan-950/30 dark:text-cyan-300' : 'border-zinc-200 text-zinc-700 dark:border-zinc-700 dark:text-zinc-200'}`}><span className="inline-block w-8 font-bold">{entry.abbr}</span><span>{fmt(entry.startDate, level >= 2)}–{fmt(entry.endDate, level >= 2)}</span><span className="float-right text-zinc-400">{entry.durationYears.toFixed(2)} y{entry.cycle ? ` · C${entry.cycle}` : ''}</span></button>; })}</div>
