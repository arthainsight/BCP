...
export default function DashaOverlap({ planets, ascendant, birthDatetime, dashaSettings }: Props) {
  ...
  return <section className="min-w-0 space-y-3 rounded-lg border border-amber-200 bg-amber-50/40 p-3 dark:border-amber-900 dark:bg-amber-950/10">
    <div className="grid min-w-0 gap-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end"><div className="min-w-0"><h3 className="text-sm font-semibold">Dasha overlap</h3><p className="text-[10px] text-zinc-500">Compare active MD, AD and PD values across all enabled systems on one date.</p></div><label className="min-w-0 text-[10px] text-zinc-500">Date<input type="date" value={date} onChange={event => setDate(event.target.value)} className="mt-1 min-h-11 w-full min-w-0 rounded border border-zinc-300 bg-white px-2 py-2 text-xs sm:mt-0 sm:ml-2 sm:w-auto dark:border-zinc-700 dark:bg-zinc-900"/></label></div>
    {overlaps.length ? <div className="grid gap-2 sm:grid-cols-2">{overlaps.map(item => <div key={item.value} className="min-w-0 rounded border border-amber-200 bg-white p-2.5 dark:border-amber-900 dark:bg-zinc-900"><div className="break-words font-semibold text-amber-800 dark:text-amber-300">{item.value} <span className="text-[9px] font-normal text-zinc-400">· {new Set(item.hits.map(hit => hit.system)).size} systems</span></div><div className="mt-1 break-words text-[10px] leading-relaxed text-zinc-500">{item.hits.map(hit => `${hit.system} ${hit.level}`).join(' · ')}</div></div>)}</div> : <p className="text-xs italic text-zinc-400">No exact values overlap between two enabled systems on this date.</p>}
    <div className="space-y-2 sm:hidden">{snapshots.map(snapshot => <div key={snapshot.key} className="min-w-0 rounded border border-amber-100 bg-white p-2.5 dark:border-amber-900 dark:bg-zinc-900"><div className="break-words text-[11px] font-semibold">{snapshot.label}</div><div className="mt-2 grid grid-cols-3 gap-2">{['MD', 'AD', 'PD'].map(level => <div key={level} className="min-w-0"><div className="text-[9px] text-zinc-400">{level}</div><div className="break-words font-mono text-[10px]">{snapshot.levels.find(item => item.level === level)?.value ?? '—'}</div></div>)}</div></div>)}</div>
    <div className="hidden overflow-x-auto sm:block"><table className="w-full text-left text-[10px]"><thead className="text-zinc-400"><tr><th className="py-1">System</th><th>MD</th><th>AD</th><th>PD</th></tr></thead><tbody>{snapshots.map(snapshot => <tr key={snapshot.key} className="border-t border-amber-100 dark:border-amber-900"><td className="py-1.5 font-medium">{snapshot.label}</td>{['MD', 'AD', 'PD'].map(level => <td key={level} className="font-mono">{snapshot.levels.find(item => item.level === level)?.value ?? '—'}</td>)}</tr>)}</tbody></table></div>
  </section>;
}
