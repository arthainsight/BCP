...
export default function DashaDateFinder({ planets, ascendant, birthDatetime, dashas, charaOptions, rasiOptions }: Props) {
  ...
  return <details className="min-w-0 rounded-xl border border-violet-200 bg-violet-50/40 dark:border-violet-900 dark:bg-violet-950/10"><summary className="min-h-11 cursor-pointer px-3 py-3 text-sm font-semibold text-violet-800 dark:text-violet-300">Dasha Date Finder</summary><div className="min-w-0 space-y-3 border-t border-violet-100 p-3 dark:border-violet-900">
    <div className="grid min-w-0 gap-2 sm:grid-cols-3"><label className="min-w-0 text-[10px] text-zinc-500">System<select value={system} onChange={event => setSystem(event.target.value as Key | 'all')} className="mt-1 min-h-11 w-full min-w-0 rounded border border-zinc-300 bg-white px-2 py-2 text-xs dark:border-zinc-700 dark:bg-zinc-900">{KEYS.map(item => <option key={item.key} value={item.key}>{item.label}</option>)}</select></label><label className="min-w-0 text-[10px] text-zinc-500">From<input type="date" value={start} onChange={event => setStart(event.target.value)} className="mt-1 min-h-11 w-full min-w-0 rounded border border-zinc-300 bg-white px-2 py-2 text-xs dark:border-zinc-700 dark:bg-zinc-900"/></label><label className="min-w-0 text-[10px] text-zinc-500">To · max 5 years<input type="date" value={end} onChange={event => setEnd(event.target.value)} className="mt-1 min-h-11 w-full min-w-0 rounded border border-zinc-300 bg-white px-2 py-2 text-xs dark:border-zinc-700 dark:bg-zinc-900"/></label></div>
    <div className="grid min-w-0 gap-2 sm:grid-cols-4"><input value={md} onChange={event => setMd(event.target.value)} placeholder="MD e.g. Saturn" aria-label="MD filter" className="min-h-11 min-w-0 rounded border border-zinc-300 bg-white px-2 py-2 text-xs dark:border-zinc-700 dark:bg-zinc-900"/><input value={ad} onChange={event => setAd(event.target.value)} placeholder="AD e.g. Mercury" aria-label="AD filter" className="min-h-11 min-w-0 rounded border border-zinc-300 bg-white px-2 py-2 text-xs dark:border-zinc-700 dark:bg-zinc-900"/><input value={pd} onChange={event => setPd(event.target.value)} placeholder="PD e.g. Jupiter" aria-label="PD filter" className="min-h-11 min-w-0 rounded border border-zinc-300 bg-white px-2 py-2 text-xs dark:border-zinc-700 dark:bg-zinc-900"/><input value={any} onChange={event => setAny(event.target.value)} placeholder="Any level" aria-label="Any level filter" className="min-h-11 min-w-0 rounded border border-zinc-300 bg-white px-2 py-2 text-xs dark:border-zinc-700 dark:bg-zinc-900"/></div>
    <div className="grid min-w-0 grid-cols-2 gap-2 sm:flex sm:flex-wrap sm:items-end"><label className="col-span-2 min-w-0 text-[10px] text-zinc-500 sm:col-span-1">Match<select value={operator} onChange={event => setOperator(event.target.value as 'and' | 'or')} className="mt-1 min-h-11 w-full min-w-0 rounded border border-zinc-300 bg-white px-2 py-2 dark:border-zinc-700 dark:bg-zinc-900"><option value="and">all filters (AND)</option><option value="or">any filter (OR)</option></select></label>{system === 'all' && <label className="text-[10px] text-zinc-500">Min. systems<input type="number" min="1" max="9" value={minimum} onChange={event => setMinimum(Math.max(1, event.target.valueAsNumber || 1))} className="mt-1 min-h-11 w-full rounded border border-zinc-300 bg-white px-2 py-2 dark:border-zinc-700 dark:bg-zinc-900"/></label>}<span className="hidden flex-1 sm:block"/><button type="button" disabled={!results.length} onClick={exportCsv} className="min-h-11 rounded border border-violet-300 px-3 py-2 text-xs text-violet-700 disabled:opacity-30 dark:border-violet-700 dark:text-violet-300">Export CSV</button><button type="button" disabled={busy} onClick={search} className="col-span-2 min-h-11 rounded bg-violet-700 px-3 py-2 text-xs font-semibold text-white disabled:opacity-40 sm:col-span-1">{busy ? 'Searching…' : 'Find dates'}</button></div>
    <div className="text-[10px] text-zinc-500">{message || 'Use exact levels or Any level. Combine active filters with AND or OR.'}</div>
    {results.length > 0 && <div className="max-h-72 space-y-2 overflow-y-auto">{results.map(result => <button type="button" key={`${result.start.toISOString()}-${result.snapshots.map(item => item.key).join('-')}`} onClick={() => select(result.start)} className="flex min-h-11 w-full min-w-0 flex-col gap-1 rounded border border-violet-100 bg-white px-2.5 py-2.5 text-left text-[10px] dark:border-violet-900 dark:bg-zinc-900"><span className="font-semibold text-violet-700 dark:text-violet-300">{fmt(result.start)}{result.end.getTime() !== result.start.getTime() ? `–${fmt(result.end)}` : ''}</span><span className="break-words leading-relaxed text-zinc-500">{result.snapshots.map(snapshot => `${snapshot.label}: ${snapshot.levels.map(level => `${level.level} ${level.value}`).join(' · ')}`).join(' | ')}</span><span className="text-zinc-400">Open timeline ↘</span></button>)}</div>}
  </div></details>;
}
