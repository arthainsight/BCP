...
// Aspect orbs are custom (not classical exact-aspect). Clamped to [0, 180].
...
function buildShadbalaRows(chart) { const ascendantSign = chart?.ascendant?.sign; return SCORE_PLANETS.map((planetName) => { const planet = getPlanet(chart, planetName); if (!planet) return null; const house = getPlanetHouse(planet, ascendantSign); const sthanaBreakdown = calculateSthanaBalaBreakdown(planetName, planet, house); const kalaBreakdown = calculateKalaBalaBreakdown(planetName, chart, planet); const naisargikaVirupa = NAISARGIKA_BALA_VIRUPA[planetName] ?? 0; const digVirupa = calculateDigBalaVirupa(planetName, house); const cheshtaVirupa = calculateCheshtaBalaVirupa(planetName, chart, planet); const drikVirupa = calculateDrikBalaVirupa(planetName, chart); const totalVirupa = sthanaBreakdown.total + digVirupa + kalaBreakdown.total + cheshtaVirupa + naisargikaVirupa + drikVirupa; const totalRupa = virupaToRupa(totalVirupa); const requiredRupa = SHADBALA_REQUIRED_RUPA[planetName] ?? 0; return { planet: planetName, house, retrograde: planet.isRetrograde ?? false, sthana: virupaToRupa(sthanaBreakdown.total), sthanaBreakdown, dig: virupaToRupa(digVirupa), digVirupa, kala: virupaToRupa(kalaBreakdown.total), kalaBreakdown, cheshta: virupaToRupa(cheshtaVirupa), cheshtaVirupa, naisargika: virupaToRupa(naisargikaVirupa), naisargikaVirupa, drik: virupaToRupa(drikVirupa), drikVirupa, total: totalRupa, totalVirupa, required: requiredRupa, ratio: requiredRupa ? totalRupa / requiredRupa : 0 }; }).filter(Boolean); }
    ...
    <div className="space-y-3">
      <div>
        ...
        <div className="text-[10px] font-mono text-zinc-400 dark:text-zinc-600 mt-1">
          Values in rūpa (÷60 from virupa). Exact: Sthāna (Uccha/Ojhayugma/Kendradi/Drekkana), Paksha, Naisargika.
          Approx: Saptavargaja (no Moolatrikona distinction), Dig (house-number proxy), Natonnata (Sun-house proxy),
          Tribhaga (fixed 6am sunrise assumed), Ayana (sin proxy{ayanamsa != null ? `, ayanamsha ${ayanamsa.toFixed(2)}°` : ''}),
          Cheṣṭā (retrograde/elongation proxy), Dṛg (orb-weighted aspects).
          ...
          <tbody>
            {shadbala.map((row) => (
              <tr key={row.planet} className="border-b border-zinc-100 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-800/50">
                ...
                <td className="p-2 border border-zinc-100 dark:border-zinc-800 text-zinc-700 dark:text-zinc-300" title={`H${row.house}`}>{formatScore(row.dig)}</td>
                <td className="p-2 border border-zinc-100 dark:border-zinc-800 text-zinc-700 dark:text-zinc-300" title={`Sect ${row.kalaBreakdown.sect}; Natonnata ${formatScore(virupaToRupa(row.kalaBreakdown.natonnata))} + Paksha ${formatScore(virupaToRupa(row.kalaBreakdown.paksha))} + Tribhaga ${formatScore(virupaToRupa(row.kalaBreakdown.tribhaaga))} (lord=${row.kalaBreakdown.tribhagaLord ?? '?'}) + Vara ${formatScore(virupaToRupa(row.kalaBreakdown.varsheshadi))} + Ayana ${formatScore(virupaToRupa(row.kalaBreakdown.ayana))}`}>{formatScore(row.kala)}</td>
                <td className="p-2 border border-zinc-100 dark:border-zinc-800 text-zinc-700 dark:text-zinc-300" title={row.retrograde ? 'retrograde → vakra=60' : ''}>{formatScore(row.cheshta)}</td>
            ...
            <table className="w-full min-w-[1200px] border-collapse text-[10px] font-mono">
              <thead>
                ...
                <tr className="bg-zinc-50 dark:bg-zinc-800/60">
                  ...
                  <th className="p-1.5 border border-zinc-200 dark:border-zinc-700 text-zinc-400 dark:text-zinc-500"></th>
                  <th className="p-1.5 border border-zinc-200 dark:border-zinc-700 text-zinc-400 dark:text-zinc-500"></th>
                  <th className="p-1.5 border border-zinc-200 dark:border-zinc-700 text-zinc-400 dark:text-zinc-500">(vp)</th>
                </tr>
              </thead>
              <tbody>
                {shadbala.map((row) => {
