...
function getDignityCellClass(dignity) {
  switch (dignity) {
    case 'exalted':
    case 'own': return 'bg-emerald-50 dark:bg-emerald-950/35 text-emerald-800 dark:text-emerald-200 border-emerald-200 dark:border-emerald-800/70';
    case 'friend': return 'bg-lime-50 dark:bg-lime-950/25 text-lime-800 dark:text-lime-200 border-lime-200 dark:border-lime-800/50';
    case 'neutral': return 'bg-zinc-50 dark:bg-zinc-800/45 text-zinc-700 dark:text-zinc-300 border-zinc-100 dark:border-zinc-800';
    case 'enemy': return 'bg-orange-50 dark:bg-orange-950/30 text-orange-800 dark:text-orange-200 border-orange-200 dark:border-orange-800/60';
    case 'debilitated': return 'bg-red-50 dark:bg-red-950/35 text-red-800 dark:text-red-200 border-red-200 dark:border-red-800/70';
    default: return 'text-zinc-700 dark:text-zinc-300 border-zinc-100 dark:border-zinc-800';
  }
}
...
function calculateSaptavargajaBalaVirupa(planetName, longitude) {
  ...
  return divisions.reduce((sum, division) => {
    ...
    return sum + (DIGNITY_VISWA[getDignity(planetName, signIndex)] ?? 0);
  }, 0) / divisions.length;
}
function calculateOjhayugmaBalaVirupa(planetName, longitude) {
  ...
  const signMatches = (signIndex) => (preference === 'odd' ? signIndex % 2 === 0 : signIndex % 2 === 1);
  return (signMatches(getVargaSign(longitude, 1)) ? 15 : 0) + (signMatches(getVargaSign(longitude, 9)) ? 15 : 0);
}
...
function calculateDrekkanaBalaVirupa(planetName, longitude) {
  ...
  const drekkana = Math.floor(getDegreesInSign(longitude) / 10) + 1;
...
function estimateSectFromSunHouse(chart) {
  const sun = (chart?.planets ?? []).find((p) => p.name === 'Sun');
  const sunHouse = getPlanetHouse(sun, chart?.ascendant?.sign);
  if (typeof sunHouse !== 'number') return 'unknown';
  return [7, 8, 9, 10, 11, 12].includes(sunHouse) ? 'day' : 'night';
}
function calculateNatonnataBalaVirupa(planetName, sect) {
  const pref = DAY_NIGHT_STRENGTH[planetName];
  if (pref === 'both') return 30;
  if (sect === 'unknown') return 0;
  return pref === sect ? 60 : 0;
}
function calculatePakshaBalaVirupa(chart) {
  const sun = (chart?.planets ?? []).find((p) => p.name === 'Sun');
  ...
  const brightVirupa = elongation <= 180 ? (elongation / 180) * 60 : ((360 - elongation) / 180) * 60;
  const darkVirupa = 60 - brightVirupa;
  return {
    Moon: brightVirupa,
    Venus: brightVirupa,
    Jupiter: brightVirupa,
    Mercury: brightVirupa,
    Sun: darkVirupa,
    Mars: darkVirupa,
    Saturn: darkVirupa,
  };
}
function calculateTribhaagaBalaVirupa(planetName, sect) {
  if (sect === 'day') return planetName === 'Mercury' ? 60 : 0;
  if (sect === 'night') return planetName === 'Moon' ? 60 : 0;
  return 0;
}
function calculateKalaBalaBreakdown(planetName, chart) {
  ...
  const tribhaaga = calculateTribhaagaBalaVirupa(planetName, sect);
  const total = natonnata + paksha + tribhaaga;
  return { sect, natonnata, paksha, tribhaaga, total };
}
...
function buildShadbalaRows(chart) {
  ...
  return SCORE_PLANETS.map((planetName) => {
    ...
    const kalaBreakdown = calculateKalaBalaBreakdown(planetName, chart);
    const naisargikaVirupa = NAISARGIKA_BALA_VIRUPA[planetName] ?? 0;
    const digVirupa = calculateDigBalaVirupa(planetName, house);
    const totalVirupa = sthanaBreakdown.total + kalaBreakdown.total + naisargikaVirupa + digVirupa;
    const totalRupa = virupaToRupa(totalVirupa);
    const requiredRupa = SHADBALA_REQUIRED_RUPA[planetName] ?? 0;
    return { planet: planetName, house, sthana: virupaToRupa(sthanaBreakdown.total), sthanaBreakdown, dig: virupaToRupa(digVirupa), kala: virupaToRupa(kalaBreakdown.total), kalaBreakdown, cheshta: null, naisargika: virupaToRupa(naisargikaVirupa), drik: null, total: totalRupa, required: requiredRupa, ratio: requiredRupa ? totalRupa / requiredRupa : 0 };
  }).filter(Boolean);
}
...
export function buildVargaMatrix(planets = {}, lagna, divisions = DEFAULT_DIVISIONS) {
  ...
  ROW_ORDER.forEach((name) => {
    ...
    divisions.forEach((division) => {
      ...
      if (typeof longitude !== 'number') { matrix[name][key] = { sign: '—', signIndex: null, dignity: 'none' }; return; }
      const signIndex = getVargaSign(longitude, division);
      matrix[name][key] = { sign: SIGN_ABBR[signIndex], signIndex, dignity: getDignity(name, signIndex) };
    });
  });
  return matrix;
}
function buildVimsopakaStrength(planets = {}, matrix = {}) {
  return SCORE_PLANETS.map((planet) => {
    if (typeof planets[planet] !== 'number') return null;
    const scores = {};
    Object.entries(VIMSOPAKA_SCHEMES).forEach(([schemeKey, scheme]) => {
      scores[schemeKey] = Object.entries(scheme.weights).reduce((sum, [divisionKey, weight]) => sum + weight * (DIGNITY_VISWA[matrix[planet]?.[divisionKey]?.dignity ?? 'none'] ?? 0) / 20, 0);
    });
    return { planet, ...scores };
  }).filter(Boolean).sort((a, b) => b.shodasavarga - a.shodasavarga || b.dasavarga - a.dasavarga || b.saptavarga - a.saptavarga || b.shadvarga - a.shadvarga);
}
function mapPlanetLongitudes(chart) {
  const wanted = new Set(ROW_ORDER.filter((name) => name !== 'Lagna'));
  const result = {};
  (chart?.planets ?? []).forEach((planet) => { if (wanted.has(planet.name) && typeof planet.longitude === 'number') result[planet.name] = planet.longitude; });
  return result;
}
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <div className="text-[10px] font-mono text-zinc-400 dark:text-zinc-600 mt-1">{divisionList} from existing sidereal longitudes. Cell colour = dignity; debilitated = 0 strength.</div>
      </div>
      <div className="overflow-x-auto border border-zinc-200 dark:border-zinc-700 rounded-lg">
        <table className="w-full min-w-[980px] border-collapse text-xs font-mono"><thead><tr className="bg-zinc-100 dark:bg-zinc-800"><th className="sticky left-0 z-10 bg-zinc-100 dark:bg-zinc-800 text-left p-2 border border-zinc-200 dark:border-zinc-700 text-zinc-500 dark:text-zinc-400">Body</th>{divisions.map((division) => <th key={division} className="text-left p-2 border border-zinc-200 dark:border-zinc-700 text-zinc-500 dark:text-zinc-400">D{division}</th>)}</tr></thead><tbody>{ROW_ORDER.map((name) => <tr key={name} className="border-b border-zinc-100 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-800/50"><td className="sticky left-0 z-10 bg-white dark:bg-zinc-900 p-2 border border-zinc-100 dark:border-zinc-800 font-bold text-emerald-700 dark:text-green-400">{name}</td>{divisions.map((division) => { const cell = matrix[name][`D${division}`]; return <td key={division} className={`p-2 border whitespace-nowrap ${getDignityCellClass(cell.dignity)}`} title={DIGNITY_LABELS[cell.dignity]}><span className="font-bold">{cell.sign}</span>{cell.dignity !== 'none' && <span className="ml-1 text-[9px] opacity-75">{DIGNITY_LABELS[cell.dignity]}</span>}</td>; })}</tr>)}</tbody></table>
      </div>

      <div className="space-y-2">
        <div><div className="text-xs font-mono text-zinc-500 dark:text-zinc-500">&gt; varga.strength.vimsopaka</div><div className="text-[10px] font-mono text-zinc-400 dark:text-zinc-600 mt-1">Four weighted calculations out of 20. Rahu/Ketu and Lagna excluded.</div></div>
        <div className="overflow-x-auto border border-zinc-200 dark:border-zinc-700 rounded-lg"><table className="w-full min-w-[720px] border-collapse text-xs font-mono"><thead><tr className="bg-zinc-100 dark:bg-zinc-800"><th className="text-left p-2 border border-zinc-200 dark:border-zinc-700 text-zinc-500 dark:text-zinc-400">Planet</th>{Object.entries(VIMSOPAKA_SCHEMES).map(([key, scheme]) => <th key={key} className="text-left p-2 border border-zinc-200 dark:border-zinc-700 text-zinc-500 dark:text-zinc-400">{scheme.label}</th>)}</tr></thead><tbody>{strength.map((row) => <tr key={row.planet} className="border-b border-zinc-100 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-800/50"><td className="p-2 border border-zinc-100 dark:border-zinc-800 font-bold text-emerald-700 dark:text-green-400">{row.planet}</td>{Object.keys(VIMSOPAKA_SCHEMES).map((key) => <td key={key} className="p-2 border border-zinc-100 dark:border-zinc-800 font-bold text-amber-700 dark:text-amber-300">{formatScore(row[key])} / 20</td>)}</tr>)}</tbody></table></div>
      </div>

      <div className="space-y-2">
        <div><div className="text-xs font-mono text-zinc-500 dark:text-zinc-500">&gt; shadbala.partial</div><div className="text-[10px] font-mono text-zinc-400 dark:text-zinc-600 mt-1">v1.30: Sthāna + Dig + Kāla + Naisargika. Kāla includes approximate sect, paksha and tribhāga; Total remains partial until Ceṣṭā and Dṛk Bala are implemented.</div></div>
        <div className="overflow-x-auto border border-zinc-200 dark:border-zinc-700 rounded-lg"><table className="w-full min-w-[920px] border-collapse text-xs font-mono"><thead><tr className="bg-zinc-100 dark:bg-zinc-800"><th className="text-left p-2 border border-zinc-200 dark:border-zinc-700 text-zinc-500 dark:text-zinc-400">Planet</th><th className="text-left p-2 border border-zinc-200 dark:border-zinc-700 text-zinc-500 dark:text-zinc-400">House</th><th className="text-left p-2 border border-zinc-200 dark:border-zinc-700 text-zinc-500 dark:text-zinc-400">Sthāna</th><th className="text-left p-2 border border-zinc-200 dark:border-zinc-700 text-zinc-500 dark:text-zinc-400">Dig</th><th className="text-left p-2 border border-zinc-200 dark:border-zinc-700 text-zinc-500 dark:text-zinc-400">Kāla</th><th className="text-left p-2 border border-zinc-200 dark:border-zinc-700 text-zinc-500 dark:text-zinc-400">Ceṣṭā</th><th className="text-left p-2 border border-zinc-200 dark:border-zinc-700 text-zinc-500 dark:text-zinc-400">Naisargika</th><th className="text-left p-2 border border-zinc-200 dark:border-zinc-700 text-zinc-500 dark:text-zinc-400">Dṛk</th><th className="text-left p-2 border border-zinc-200 dark:border-zinc-700 text-zinc-500 dark:text-zinc-400">Partial Total</th><th className="text-left p-2 border border-zinc-200 dark:border-zinc-700 text-zinc-500 dark:text-zinc-400">Required</th><th className="text-left p-2 border border-zinc-200 dark:border-zinc-700 text-zinc-500 dark:text-zinc-400">Ratio</th></tr></thead><tbody>{shadbala.map((row) => <tr key={row.planet} className="border-b border-zinc-100 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-800/50"><td className="p-2 border border-zinc-100 dark:border-zinc-800 font-bold text-emerald-700 dark:text-green-400">{row.planet}</td><td className="p-2 border border-zinc-100 dark:border-zinc-800 text-zinc-700 dark:text-zinc-300">{row.house ? `H${row.house}` : '—'}</td><td className="p-2 border border-zinc-100 dark:border-zinc-800 text-zinc-700 dark:text-zinc-300" title={`Uccha ${formatScore(virupaToRupa(row.sthanaBreakdown.uccha))} + Saptavargaja ${formatScore(virupaToRupa(row.sthanaBreakdown.saptavargaja))} + Ojhayugma ${formatScore(virupaToRupa(row.sthanaBreakdown.ojhayugma))} + Kendradi ${formatScore(virupaToRupa(row.sthanaBreakdown.kendradi))} + Drekkana ${formatScore(virupaToRupa(row.sthanaBreakdown.drekkana))}`}>{formatScore(row.sthana)}</td><td className="p-2 border border-zinc-100 dark:border-zinc-800 text-zinc-700 dark:text-zinc-300">{formatScore(row.dig)}</td><td className="p-2 border border-zinc-100 dark:border-zinc-800 text-zinc-700 dark:text-zinc-300" title={`Sect ${row.kalaBreakdown.sect}; Natonnata ${formatScore(virupaToRupa(row.kalaBreakdown.natonnata))} + Paksha ${formatScore(virupaToRupa(row.kalaBreakdown.paksha))} + Tribhaga ${formatScore(virupaToRupa(row.kalaBreakdown.tribhaaga))}`}>{formatScore(row.kala)}</td><td className="p-2 border border-zinc-100 dark:border-zinc-800 text-zinc-400 dark:text-zinc-600">—</td><td className="p-2 border border-zinc-100 dark:border-zinc-800 text-zinc-700 dark:text-zinc-300">{formatScore(row.naisargika)}</td><td className="p-2 border border-zinc-100 dark:border-zinc-800 text-zinc-400 dark:text-zinc-600">—</td><td className="p-2 border border-zinc-100 dark:border-zinc-800 font-bold text-amber-700 dark:text-amber-300">{formatScore(row.total)}</td><td className="p-2 border border-zinc-100 dark:border-zinc-800 text-zinc-700 dark:text-zinc-300">{formatScore(row.required)}</td><td className="p-2 border border-zinc-100 dark:border-zinc-800 font-bold text-cyan-700 dark:text-cyan-300">{formatScore(row.ratio)}</td></tr>)}</tbody></table></div>
      </div>
    </div>
  );
}
