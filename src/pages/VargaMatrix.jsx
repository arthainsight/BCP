...
// Mūlatrikoṇa only appears when getDignity is given a degree, which the varga
// matrix does not do — its cells are whole-sign. The entries are here so the
// maps stay total against the Dignity union.
const DIGNITY_VISWA = { exalted: 20, moolatrikona: 20, own: 20, friend: 15, neutral: 10, enemy: 7, debilitated: 0, none: 0 };
const DIGNITY_LABELS = { exalted: 'Ex', moolatrikona: 'MT', own: 'Own', friend: 'Fr', neutral: 'Neu', enemy: 'En', debilitated: 'Deb', none: '—' };
...
function getDignityCellClass(dignity) { switch (dignity) { case 'exalted': case 'moolatrikona': case 'own': return 'bg-emerald-50 dark:bg-emerald-950/35 text-emerald-800 dark:text-emerald-200 border-emerald-200 dark:border-emerald-800/70'; case 'friend': return 'bg-lime-50 dark:bg-lime-950/25 text-lime-800 dark:text-lime-200 border-lime-200 dark:border-lime-800/50'; case 'neutral': return 'bg-zinc-50 dark:bg-zinc-800/45 text-zinc-700 dark:text-zinc-300 border-zinc-100 dark:border-zinc-800'; case 'enemy': return 'bg-orange-50 dark:bg-orange-950/30 text-orange-800 dark:text-orange-200 border-orange-200 dark:border-orange-800/60'; case 'debilitated': return 'bg-red-50 dark:bg-red-950/35 text-red-800 dark:text-red-200 border-red-200 dark:border-red-800/70'; default: return 'text-zinc-700 dark:text-zinc-300 border-zinc-100 dark:border-zinc-800'; } }
...
function formatClock(decimalHours) {
  ...
  return `${String((hours + (carry ? 1 : 0)) % 24).padStart(2, '0')}:${String(carry ? 0 : minutes).padStart(2, '0')}`;
}
    ...
    <div className="space-y-3">
      <div>
        ...
        <div className="text-[10px] font-mono text-zinc-400 dark:text-zinc-600 mt-1">
          Values in virūpa. Exact: Uccha, Saptavargaja, Ojhayugma, Kendradi, Drekkana, Natonnata,
          Paksha, Tribhaga, Vara, Ayana, Cheṣṭā, Naisargika.
          Approx (~): Dig (whole-sign house proxy), Dṛg (orb-weighted, can be negative).
          Ayana from true declination{ayanamsa != null ? `; ayan ${ayanamsa.toFixed(2)}°` : ''}. Cheṣṭā from the
          eightfold motion; the Sun takes its Ayana Bala and the Moon its Pakṣa Bala.
          {sunrise != null && sunset != null
            ? ` Sunrise ${formatClock(sunrise)}, sunset ${formatClock(sunset)} drive Natonnata and Tribhaga.`
            : ' No sunrise data on this chart — Natonnata and Tribhaga fall back to a 6am/6pm horizon.'}
            ...
            {shadbala.map((row) => {
              ...
              return (
                <tr key={row.planet} className="border-b border-zinc-100 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-800/50">
                  ...
                  <td className="p-2 border border-zinc-100 dark:border-zinc-800 text-zinc-700 dark:text-zinc-300" title={row.retrograde ? 'Cheṣṭā: retrograde → vakra = 60 vp' : 'Cheṣṭā: eightfold motion vs mean daily speed'}>{formatScore(row.cheshtaVirupa)}</td>
